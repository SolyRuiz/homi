# -*- coding: utf-8 -*-
# Part of Tribuc. See LICENSE file for full copyright and licensing details.
...
class pos_config(models.Model):
	...
	pos_display_stock = fields.Boolean(string='Mostrar cantidad en el POS')
	pos_stock_type = fields.Selection(
		[('onhand', 'Cantidad a mano'), ('incoming', 'Cantidad entrante'), ('outgoing', 'Cantidad saliente'),
		 ('available', 'Cantidad Disponible')], string='Tipo de cantidad', help='Vendedor puede mostrar diferentes cantidades en POS.')
	pos_allow_order = fields.Boolean(string=' Orden de POS cuando el producto esta fuera de existencias')
	pos_deny_order = fields.Char(string='Denegar pedido de POS cuando la cantidad del producto se reduce a')
	...
	stock_location_id = fields.Many2one(
		'stock.location', string='ubicacion de existencias',
...
class ProductInherit(models.Model):
	...
	quant_text = fields.Text('Cantidad', compute='_compute_avail_locations', store=True)
	...
	def _compute_avail_locations(self):
		...
		for rec in self:
			...
			if rec.type == 'product' :
				...
				outgoing = self.env['stock.move'].sudo().search([('product_id', '=', rec.id), ('state', 'not in', ['done']),
																 ('location_id.usage', '=', 'internal'),
																 ('picking_id.picking_type_code', 'in', ['outgoing'])])
				incoming = self.env['stock.move'].sudo().search([('product_id', '=', rec.id), ('state', 'not in', ['done']),
